...
namespace UWash
{
	...
	{
		private static string[] RolePrefixes = new string[]
		{
			"Actor ",
			"Actress ",
			"Singer ",
			...
			"Silent film actor ",
			"Silent film actress ",
			"Radio xylophone soloist ",
		};

		public static void ProduceSayreCategories(Api commonsApi, CategoryTree catTree,
			int key, CategoryMapping categoryMappings,
			Dictionary<string, string> metadata, HashSet<PageTitle> categories)
		{
			...
			// pull out any possible names from the title
			...
			if (RolePrefixes.Any(prefix => title.StartsWith(prefix, StringComparison.OrdinalIgnoreCase)))
			{
			...
			foreach (string name in maybeNames)
			{
				...
				MappingCategory mapping = categoryMappings.TryGetValue(name);
				if (mapping != null)
				{
					if (!mapping.IsUnmapped)
					{
						categories.Add(PageTitle.Parse(mapping.MappedCategory));
					}
				}

				// try mapping
				mapping = categoryMappings.TryMapValue(name, keyString);

				string nametrim = name.Trim();
				foreach (string role in RolePrefixes)
				{
					if (nametrim.StartsWith(role, StringComparison.OrdinalIgnoreCase))
					{
						nametrim = nametrim.Substring(role.Length);
					}
				}
				...
				if (personCategory != null && !personCategory.missing)
				{
					...
					{
						//TODO: check lifetime?
						mapping.MappedCategory = personCategory.title.ToString();
